...
import com.star4droid.star2d.Items.CircleBody;
import com.star4droid.star2d.Items.Editor;
import com.star4droid.star2d.Items.EditorItem;
import com.star4droid.star2d.Items.JoyStickItem;
import com.star4droid.star2d.Items.ProgressItem;
import com.star4droid.star2d.Items.TextItem;
            ...
            public void run() {
                ...
                for (String path : joints) {
                    ...
                    if (p.contains("-")) {
                        ...
                        ArrayList<HashMap<String, Object>> fields = new Gson().fromJson(FileUtil.readFile(path), new TypeToken<ArrayList<HashMap<String, Object>>>() {
                        }.getType());
                        ...
                        joint.append(String.format(defualtJointCode, p.split("-")[1], p.split("-")[0], init));
                    }
                    ...
                    if (Utils.isEditorItem(child)) {
                        ...
                        try {
                            ...
                            for (Map.Entry<String, Object> entry : propertySet.entrySet()) {
                                //ignored properties...
                                if ("(TYPE)(lock)(Script)(parent)".contains("(" + entry.getKey() + ")"))
                                    continue;
                ...
                itemsCode += "\n" + joint + editor.readEvent("OnCreate");
                if (!vars.equals("")) vars += ";";
                ...
                if (generateListener != null) {
                    new Handler(Looper.getMainLooper()).post(() -> {
                        ...
                        generateListener.onGenerate(String.format(playerCode, editor.getScene().toLowerCase(), variables, code, editor.readEvent("onPause"), editor.readEvent("onResume"), editor.readEvent("onStep"), editor.readEvent("onCollisionStart"), editor.readEvent("onCollisionEnd"), scriptBuilder));
                    });
                }
            }
        }.start();
